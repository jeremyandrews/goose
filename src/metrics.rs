...
pub struct GooseRequestMetric {
    ...
    pub redirected: bool,
    /// Time to First Byte (TTFB) for this request in milliseconds.
    ///
    /// TTFB measures the time from when the request was sent until the first
    /// byte of the response was received (when headers arrive). This is useful
    /// for understanding server processing time vs. data transfer time.
    ///
    /// TTFB will always be ≤ total response_time. For requests that don't read
    /// the response body, TTFB may equal the total response time.
...
impl GooseRequestMetricAggregate {
    ...
    /// Record TTFB (Time to First Byte) time.
    pub(crate) fn record_ttfb_time(
        &mut self,
        ttfb_elapsed: u64,
        coordinated_omission_mitigation: bool,
    ) {
        // Only add ttfb_elapsed to raw_data if the time wasn't generated by Coordinated
        // Omission Mitigation.
        if !coordinated_omission_mitigation {
            self.raw_data.record_ttfb_time(ttfb_elapsed);
        }

        // A Coordinated Omission data object already exists, add a new time into the data.
        if let Some(coordinated_omission_data) = self.coordinated_omission_data.as_mut() {
            coordinated_omission_data.record_ttfb_time(ttfb_elapsed);
        }
        // Create a new Coordinated Omission data object by cloning the raw data.
        else if coordinated_omission_mitigation {
            // If this ttfb_elapsed was generated by Coordinated Omission Mitigation, it doesn't
            // exist in the raw_data, so add it.
            let mut coordinated_omission_data = self.raw_data.clone();
            coordinated_omission_data.record_ttfb_time(ttfb_elapsed);
            self.coordinated_omission_data = Some(coordinated_omission_data);
...
pub struct GooseRequestMetricTimingData {
    ...
    pub counter: usize,
    /// Per-TTFB-time counters, tracking how often pages return with this TTFB.
    ///
    /// TTFB (Time To First Byte) measures the time from request start until the first
    /// byte of the response is received (headers arrive). Uses the same rounding logic
    /// as response times for consistency.
    pub ttfb_times: BTreeMap<usize, usize>,
    /// The shortest TTFB seen so far.
    ///
    /// For example a `ttfb_minimum_time` of `5` means the quickest TTFB for this method-path
    /// pair was 5 milliseconds. This value is not rounded.
    pub ttfb_minimum_time: usize,
    /// The longest TTFB seen so far.
    ///
    /// For example a `ttfb_maximum_time` of `1500` means the slowest TTFB for this method-path
    /// pair was 1500 milliseconds. This value is not rounded.
    pub ttfb_maximum_time: usize,
    /// Total combined TTFB times seen so far.
    ///
    /// A running total of all TTFB times for this method-path pair.
    pub ttfb_total_time: usize,
}
impl GooseRequestMetricTimingData {
    ...
    /// Record a new TTFB time.
    pub(crate) fn record_ttfb_time(&mut self, ttfb_elapsed: u64) {
        // Perform this conversion only once, then re-use throughout this function.
        let ttfb = ttfb_elapsed as usize;

        // Update minimum if this one is fastest yet.
        if ttfb > 0 && (self.ttfb_minimum_time == 0 || ttfb < self.ttfb_minimum_time) {
            self.ttfb_minimum_time = ttfb;
        }

        // Update maximum if this one is slowest yet.
        if ttfb > self.ttfb_maximum_time {
            self.ttfb_maximum_time = ttfb;
        }

        // Update total TTFB time, adding in this one.
        self.ttfb_total_time += ttfb;

        // Use the same rounding logic as response times for consistency.
        let rounded_ttfb = if ttfb_elapsed < 100 {
            ttfb
        }
        // Round to nearest 10 for 100-500ms times.
        else if ttfb_elapsed < 500 {
            ((ttfb_elapsed as f64 / 10.0).round() * 10.0) as usize
        }
        // Round to nearest 100 for 500-1000ms times.
        else if ttfb_elapsed < 1000 {
            ((ttfb_elapsed as f64 / 100.0).round() * 100.0) as usize
        }
        // Round to nearest 1000 for all larger times.
        else {
            ((ttfb_elapsed as f64 / 1000.0).round() * 1000.0) as usize
        };

        let counter = match self.ttfb_times.get(&rounded_ttfb) {
            // We've seen this TTFB time before, increment counter.
            Some(c) => {
                debug!("got {rounded_ttfb:?} TTFB counter: {c}");
                *c + 1
            }
            // First time we've seen this TTFB time, initialize counter.
            None => {
                debug!("no match for TTFB counter: {rounded_ttfb}");
                1
            }
        };
        debug!("incremented {rounded_ttfb} TTFB counter: {counter}");
        self.ttfb_times.insert(rounded_ttfb, counter);
    }
}
...
impl GooseMetrics {
    ...
    pub(crate) fn fmt_response_times(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        // Display TTFB metrics table
        let mut aggregate_raw_ttfb_times: BTreeMap<usize, usize> = BTreeMap::new();
        let mut aggregate_raw_total_ttfb_time: usize = 0;
        let mut aggregate_raw_ttfb_counter: usize = 0;
        let mut aggregate_raw_min_ttfb_time: usize = 0;
        let mut aggregate_raw_max_ttfb_time: usize = 0;
        writeln!(
            fmt,
            " ------------------------------------------------------------------------------"
        )?;
        writeln!(fmt, " Time to First Byte (TTFB) Metrics:")?;
        writeln!(
            fmt,
            " ------------------------------------------------------------------------------"
        )?;
        ...
        // Display TTFB data for each request.
        for (request_key, request) in self.requests.iter().sorted() {
            let ttfb_average = match request.raw_data.counter {
                0 => 0.0,
                _ => request.raw_data.ttfb_total_time as f32 / request.raw_data.counter as f32,
            };
            let ttfb_average_precision = determine_precision(ttfb_average);

            // Merge in all TTFB times from this request into an aggregate.
            aggregate_raw_ttfb_times = merge_times(
                aggregate_raw_ttfb_times,
                request.raw_data.ttfb_times.clone(),
            );
            // Increment total TTFB time counter.
            aggregate_raw_total_ttfb_time += &request.raw_data.ttfb_total_time;
            // Increment counter tracking individual TTFB times seen.
            aggregate_raw_ttfb_counter += &request.raw_data.counter;
            // If user had new fastest TTFB time, update global fastest TTFB time.
            aggregate_raw_min_ttfb_time = update_min_time(
                aggregate_raw_min_ttfb_time,
                request.raw_data.ttfb_minimum_time,
            );
            // If user had new slowest TTFB time, update global slowest TTFB time.
            aggregate_raw_max_ttfb_time = update_max_time(
                aggregate_raw_max_ttfb_time,
                request.raw_data.ttfb_maximum_time,
            );

            writeln!(
                fmt,
                " {:<24} | {:>11.ttfb_avg_precision$} | {:>10} | {:>11} | {:>10}",
                util::truncate_string(request_key, 24),
                ttfb_average,
                format_number(request.raw_data.ttfb_minimum_time),
                format_number(request.raw_data.ttfb_maximum_time),
                format_number(util::median(
                ...
                ttfb_avg_precision = ttfb_average_precision,
            )?;
        }

        let ttfb_average = match aggregate_raw_ttfb_counter {
            0 => 0.0,
            _ => aggregate_raw_total_ttfb_time as f32 / aggregate_raw_ttfb_counter as f32,
        };
        let ttfb_average_precision = determine_precision(ttfb_average);

        // Display aggregated TTFB data if there was more than one request.
        if self.requests.len() > 1 {
            writeln!(
                fmt,
                " -------------------------+-------------+------------+-------------+-----------"
            ...
            writeln!(
                ...
                "Aggregated",
                ttfb_average,
                format_number(aggregate_raw_min_ttfb_time),
                format_number(aggregate_raw_max_ttfb_time),
                format_number(util::median(
                    &aggregate_raw_ttfb_times,
                    aggregate_raw_ttfb_counter,
                    aggregate_raw_min_ttfb_time,
                    aggregate_raw_max_ttfb_time
                )),
                avg_precision = ttfb_average_precision,
            )?;
        }
...
impl GooseAttack {
    ...
    async fn record_request_metric(&mut self, request_metric: &GooseRequestMetric) {
        ...
        // Store a new metric.
            ...
            // Record TTFB time for this request.
            merge_request.record_ttfb_time(
                request_metric.time_to_first_byte,
                request_metric.coordinated_omission_elapsed > 0,
            );
                ...
                GooseMetric::Request(request_metric) => {
                    ...
                    // Otherwise this is an actual request, record it normally.
                        ...
                        if !self.configuration.report_file.is_empty() {
                            ...
                            self.graph_data.record_average_ttfb_time_per_second(
                                key.clone(),
                                seconds_since_start,
                                request_metric.time_to_first_byte,
                            );
    ...
    pub(crate) async fn write_html_report(
        ...
        for metric in &raw_response_metrics {
            raw_responses_rows.push(report::response_metrics_row(metric.clone()));
        }

        // Compile the TTFB metrics template.
        let mut raw_ttfb_rows = Vec::new();
        for metric in &raw_response_metrics {
            raw_ttfb_rows.push(report::ttfb_metrics_row(metric.clone()));
        }
        ...
        let report = report::build_report(
            ...
            report::GooseReportTemplates {
                ...
                raw_ttfb_template: &raw_ttfb_rows.join("\n"),
